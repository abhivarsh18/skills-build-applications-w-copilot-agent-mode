...
router.get('/teams', async (_request, response, next) => {
  try {
    response.json(await Team.find().populate('members', 'name email').sort({ createdAt: -1 }));
  } catch (error) {
    next(error);
  }
});
...
router.get('/leaderboard', async (_request, response, next) => {
  try {
    ...
    response.json(leaderboard.map((entry, index) => ({ ...entry, rank: index + 1 })));
  } catch (error) {
    next(error);
  }
});
...
router.get('/leaderboard/snapshots', async (_request, response, next) => {
  try {
